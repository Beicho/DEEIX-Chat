...
export function AdminBillingPage() {
  ...
  const loadPaymentOrders = React.useCallback(async () => {
    ...
    try {
      ...
      toast.error(t("toast.paymentOrdersLoadFailed"), { description: resolveAdminErrorMessage(error) });
    } finally {
  ...
  const loadBalanceTransactions = React.useCallback(async () => {
    ...
    try {
      ...
      toast.error(t("toast.balanceTransactionsLoadFailed"), { description: resolveAdminErrorMessage(error) });
    } finally {
  ...
  const loadRiskSummary = React.useCallback(async () => {
    ...
    try {
      ...
      toast.error(t("toast.riskLoadFailed"), { description: resolveAdminErrorMessage(error) });
    } finally {
  ...
  async function exportBillingUsageCSV() {
    ...
    try {
      ...
      toast.error(t("toast.billingCSVExportFailed"), { description: resolveAdminErrorMessage(error) });
    } finally {
  ...
  async function createPlan(event?: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      toast.error(t("toast.planCreateFailed"), { description: resolveAdminErrorMessage(error) });
    } finally {
      setSaving(false);
    }
  }
  ...
  async function deletePlan(plan: AdminBillingPlanDTO) {
    ...
    try {
      ...
      toast.error(t("toast.planDeleteFailed"), { description: resolveAdminErrorMessage(error) });
    } finally {
      setSaving(false);
    }
  }
  ...
  async function applyPaymentAction(order: AdminPaymentOrderDTO, action: "complete" | "expire" | "fail") {
    ...
    try {
      ...
      toast.error(t("toast.paymentOrderUpdateFailed"), { description: resolveAdminErrorMessage(error) });
    } finally {
  ...
  async function applyBalanceDelta(event?: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      toast.error(t("toast.balanceDeltaFailed"), { description: resolveAdminErrorMessage(error) });
    } finally {
      setSaving(false);
    }
  }
    ...
    <div className="space-y-8 pb-10">
      ...
      <section className="space-y-6 px-1">
        ...
        <Table>
          ...
          <TableBody>
            {paymentOrderLoading ? <TableLoadingRow colSpan={5} /> : null}
      ...
      <section className="grid gap-6 px-1 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
        <div className="space-y-4">
          ...
          <Table>
            ...
            <TableBody>
              {balanceTransactionLoading ? <TableLoadingRow colSpan={3} /> : null}
