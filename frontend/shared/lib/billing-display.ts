...
const DEFAULT_BILLING_DISPLAY_LABELS: BillingDisplayLabels = {
  ...
  openaiServiceTierNote: (tier, multiplier) => `OpenAI service tier ${tier} bills at ${multiplier}`,
...
export function formatUSDWithCNY(usd: number, locale: string): string {
  ...
  if (locale.startsWith("zh")) {
    const cny = usd * CNY_EXCHANGE_RATE;
    ...
    return usdFormatted + " (约 " + cnyFormatted + ")";
  }
  return usdFormatted;
}
