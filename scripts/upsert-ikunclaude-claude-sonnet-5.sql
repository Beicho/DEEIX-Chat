...
-- Add/update platform model claude-sonnet-5 and route it through the existing
-- IkunClaude upstream. Pricing is filled directly in USD using OpenRouter's
-- published anthropic/claude-sonnet-5 token prices.
--
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM llm_upstreams WHERE lower(name) = lower('IkunClaude')) THEN
...
WITH up AS (
  SELECT id
  FROM llm_upstreams
  WHERE lower(name) = lower('IkunClaude')
  ORDER BY id
  LIMIT 1
),
platform_model AS (
  ...
  VALUES (
    ...
    'public',
    'claude',
    'Anthropic Claude Sonnet 5 via IkunClaude; prices sourced from OpenRouter',
    'active'
  )
...
upstream_model AS (
  ...
  SELECT
    ...
    now(),
    up.id,
    'upm_ikunclaude_claude_sonnet_5',
    'claude-sonnet-5',
    'anthropic',
    'claude',
    'anthropic_messages',
    ...
    'manual',
    now(),
    '{"pricing_source":"openrouter","openrouter_model_id":"anthropic/claude-sonnet-5","openrouter_pricing":{"prompt":"0.000002","completion":"0.00001","web_search":"0.01","input_cache_read":"0.0000002","input_cache_write":"0.0000025","input_cache_write_1h":"0.000004"}}'
  FROM up
