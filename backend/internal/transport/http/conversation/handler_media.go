...
func (h *Handler) StreamVideoGeneration(c *gin.Context) {
	...
	if err != nil {
		if releaseErr := h.releaseSendMessageUsageReservation(reservation, "视频生成失败退回预扣"); releaseErr != nil {
			_ = flushStreamEvent(billingStreamErrorPayload(releaseErr))
			h.service.FinishMessageGeneration(req.ClientRunID)
			return
		}
	...
	usageLedger, billingErr := h.service.RecordSendMessageBilling(
		billingCtx,
		mediaVideoBillingInput(userID, conversation, &req, result),
		reservation,
	)
	billingCancel()
	if billingErr != nil {
...
// mediaVideoBillingInput 构造视频任务计费上下文。
...
// ensureMediaVideoBillingModelAccess 校验视频模型计费权限。
...
// reserveMediaVideoUsageBalance 预扣视频生成余额。
