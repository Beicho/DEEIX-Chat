...
func (r *Repo) GetAdminCheckInStats(ctx context.Context, activeSince time.Time) (*domainbilling.AdminCheckInStats, error) {
	...
	activeStart := dateOnly(activeSince)
	todayStart := activeStart.AddDate(0, 0, 6)
	if err := r.db.WithContext(ctx).
		Model(&model.CheckInRecord{}).
		Where("check_in_date >= ? AND check_in_date < ?", todayStart, todayStart.AddDate(0, 0, 1)).
		Distinct("user_id").
	...
	if err := r.db.WithContext(ctx).
		Model(&model.CheckInRecord{}).
		Where("check_in_date >= ?", activeStart).
		Distinct("user_id").
		Count(&stats.ActiveUsersLast7Days).Error; err != nil {
		return nil, translateError(err)
	}
	totalSQL := `
		SELECT
...
func (r *Repo) ListRedemptionRecords(ctx context.Context, filter repository.RedemptionListFilter, offset int, limit int) ([]repository.RedemptionRecord, int64, error) {
	if limit <= 0 {
		limit = 20
	}
	if limit > 200 {
