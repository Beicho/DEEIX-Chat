...
func TestListRedemptionsJoinsCodeAndBalanceContext(t *testing.T) {
	...
	items, total, err := repo.ListRedemptionRecords(ctx, repository.RedemptionListFilter{}, 0, 20)
	if err != nil {
	...
	balanceItems, _, err := repo.ListRedemptionRecords(ctx, repository.RedemptionListFilter{UserID: 7}, 0, 20)
	if err != nil {
	...
	deletedItems, _, err := repo.ListRedemptionRecords(ctx, repository.RedemptionListFilter{CodeID: deletedCode.ID}, 0, 20)
	if err != nil {
	...
	keywordItems, _, err := repo.ListRedemptionRecords(ctx, repository.RedemptionListFilter{Query: "bbbb"}, 0, 20)
	if err != nil {
	...
	comboItems, comboTotal, err := repo.ListRedemptionRecords(ctx, repository.RedemptionListFilter{Query: "aaaa", UserID: 7}, 0, 20)
	if err != nil {
	...
	rewardItems, _, err := repo.ListRedemptionRecords(ctx, repository.RedemptionListFilter{RewardType: domainbilling.RedemptionRewardTypeSubscription}, 0, 20)
	if err != nil {
	...
	rangedItems, _, err := repo.ListRedemptionRecords(ctx, repository.RedemptionListFilter{CreatedFrom: &from, Sort: "created_asc"}, 0, 20)
	if err != nil {
