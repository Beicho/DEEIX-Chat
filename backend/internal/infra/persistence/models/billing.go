...
func (CheckInRecord) TableName() string {
	return "billing_checkin_records"
}

// TaskProgress 预留用户任务奖励进度。
type TaskProgress struct {
	BaseModel
	UserID               uint       `gorm:"not null;uniqueIndex:idx_billing_task_progress_user_task,priority:1;index:idx_billing_task_progress_user_id;comment:用户ID"`
	TaskKey              string     `gorm:"size:64;not null;uniqueIndex:idx_billing_task_progress_user_task,priority:2;index:idx_billing_task_progress_task_key;comment:任务键"`
	Progress             int        `gorm:"not null;default:0;comment:当前进度"`
	Target               int        `gorm:"not null;default:1;comment:目标进度"`
	Status               string     `gorm:"size:32;not null;default:'pending';index:idx_billing_task_progress_status;comment:任务状态"`
	RewardNanousd        int64      `gorm:"not null;default:0;comment:奖励金额(纳美元)"`
	BalanceTransactionID uint       `gorm:"not null;default:0;index:idx_billing_task_progress_balance_tx_id;comment:余额流水ID"`
	CompletedAt          *time.Time `gorm:"comment:完成时间"`
}

// TableName 指定表名。
func (TaskProgress) TableName() string {
	return "billing_task_progress"
}

// ExternalAccountLink 记录本站用户与外部平台账号绑定。
...
// TableName 指定表名。
func (ExternalAccountLink) TableName() string {
	return "external_account_links"
}

// ExternalTransfer 记录外部余额单向划转。
