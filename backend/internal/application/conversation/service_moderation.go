...
func (s *Service) moderationHTTPClient(cfg moderationRuntimeConfig) *http.Client {
	timeout := time.Duration(cfg.TimeoutSeconds) * time.Second
	if timeout <= 0 {
		timeout = 10 * time.Second
	}
	snapshot := s.cfg.Snapshot()
	return security.NewOutboundHTTPClient(snapshot.Env, snapshot.SSRFProtectionEnabled, timeout)
}
...
func (s *Service) checkOpenAIModeration(ctx context.Context, cfg moderationRuntimeConfig, direction string, content string) (*moderationCheckResult, error) {
	...
	resp, err := s.moderationHTTPClient(cfg).Do(req)
	if err != nil {
		return nil, err
	}
...
func (s *Service) recordModerationEvent(ctx context.Context, input SendMessageInput, messageID uint, runID string, direction string, result *moderationCheckResult, reason string) {
	if result == nil {
		return
	}
	...
	if err := s.repo.CreateModerationEvent(ctx, event); err == nil && event.Flagged {
		_ = s.applyModerationAutoDisposition(ctx, input.UserID)
	}
}
...
func (s *Service) applyModerationAutoDisposition(ctx context.Context, userID uint) error {
	...
	if cfg.AutoSuspendThreshold > 0 && count >= int64(cfg.AutoSuspendThreshold) {
		...
		if err = s.userEnforcer.SetUserSuspension(ctx, userID, "content_policy", "content_policy_auto_suspension", &now, nil); err != nil {
			return err
		}
		return s.userEnforcer.RevokeAllSessions(ctx, userID, "content_policy_auto_suspension")
	}
	if cfg.AutoLimitThreshold > 0 && count >= int64(cfg.AutoLimitThreshold) && user.Status == domainuser.StatusActive {
		if err = s.userEnforcer.UpdateUserStatus(ctx, userID, domainuser.StatusLocked); err != nil {
			return err
		}
		return s.userEnforcer.SetUserSuspension(ctx, userID, "content_policy", "content_policy_auto_limit", &now, nil)
	}
	return nil
}
...
func (s *Service) checkContentForPolicy(ctx context.Context, userID uint, conversationID uint, messageID uint, runID string, content string, direction string) error {
	...
	if err != nil {
		s.recordModerationEvent(ctx, input, messageID, runID, direction, s.moderationFailureResult(err), "check_failed")
	...
	if result != nil && result.Flagged {
		s.recordModerationEvent(ctx, input, messageID, runID, direction, result, "blocked")
		return ErrModerationBlocked
	}
	return nil
}
