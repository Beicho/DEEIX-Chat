...
func defaultSettings() []domainsettings.SystemSetting {
	return []domainsettings.SystemSetting{
		...
		{Namespace: "auth", Key: "public_auth_rate_limit_enabled", Value: "true", ValueType: "bool", Description: "启用登录/注册等公开接口限流（与站内聊天限流开关独立）"},
		...
		{Namespace: "auth", Key: "invite_registration_required", Value: "false", ValueType: "bool", Description: "注册是否要求邀请码"},
		{Namespace: "auth", Key: "invite_provider_registration", Value: "true", ValueType: "bool", Description: "第三方登录首次注册是否同样要求邀请码"},
		...
		{Namespace: "risk", Key: "cost_guard_enabled", Value: "true", ValueType: "bool", Description: "调用前校验剩余额度与余额是否覆盖本次预估成本"},
		{Namespace: "risk", Key: "new_user_cooldown_hours", Value: "24", ValueType: "int", Description: "新注册账号冷却小时数，期间禁用高价模型，0 表示关闭"},
		{Namespace: "risk", Key: "new_user_cooldown_cost_usd", Value: "1", ValueType: "string", Description: "冷却期内禁止的单次预估成本阈值（美元）"},
		{Namespace: "risk", Key: "daily_spend_limit_usd", Value: "0", ValueType: "string", Description: "单用户单日消费上限（美元），0 表示不限"},
		{Namespace: "risk", Key: "max_concurrent_generations", Value: "3", ValueType: "int", Description: "单用户同时进行的生成任务上限，0 表示不限"},
		{Namespace: "risk", Key: "auto_suspend_debt_usd", Value: "5", ValueType: "string", Description: "欠费超过该金额自动停用账号（美元），0 表示关闭"},
		{Namespace: "risk", Key: "spend_alert_single_call_usd", Value: "5", ValueType: "string", Description: "单次调用消费告警阈值（美元），0 表示关闭"},
		{Namespace: "risk", Key: "spend_alert_daily_user_usd", Value: "50", ValueType: "string", Description: "单用户单日消费告警阈值（美元），0 表示关闭"},
		{Namespace: "risk", Key: "fingerprint_alert_enabled", Value: "true", ValueType: "bool", Description: "检测到高危多账号关联时发送告警（仅提醒人工复核）"},
		{Namespace: "risk", Key: "fingerprint_alert_min_accounts", Value: "5", ValueType: "int", Description: "触发多账号告警的最少关联账号数，低于该值不打扰"},
		{Namespace: "risk", Key: "fingerprint_auto_suspend", Value: "0", ValueType: "int", Description: "同一设备关联账号数达到该值自动停用，0 表示关闭。注意：同机场/同代理出口或同型号手机会产生相同特征，容易误封，建议保持 0 并人工复核"},
